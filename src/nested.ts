...
export function toCSV(questions: Question[]): string {
    ...
    const dataRows = questions.map((question: Question): string => {
        const optionsCount = question.options.length;
        const published = question.published ? "true" : "false";
        return `${question.id},${question.name},${optionsCount},${question.points},${published}`;
    });
...
export function duplicateQuestionInArray(
    ...
    return questions.flatMap((question: Question): Question[] | Question => {
        if (question.id === targetId) {
